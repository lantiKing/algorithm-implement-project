...
listpfs(){
  int k;
  for(k=1; k<=V; k++) val[k]=-unseen;//初期化（すべて未訪問だ）
  for(k=1; k<=V; k++) if(val[k]==-unseen) visit(k);//まだ訪問してないなら訪問する
...
//dataが存在して、かつpriorityが新しいpriorityより大きい->優先度を更新する return 1
//dataが存在しない->insert　return 1
//どちらでもない->何もしない return 0
